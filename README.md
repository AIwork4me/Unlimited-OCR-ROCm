...
<p align="center">
  OmniDocBench v1.6 Overall 92.451 (PyTorch fast path) · gate PASS · 1.88× lossless speedup · 16 GB VRAM · R-SWA constant memory
</p>
...
|---|---|---|---|
| **92.451 Overall** ✓ _(PyTorch fast path)_ | **PASS** | **1.88× lossless** · ~0.21 pp/s (4-GPU) | **16 GB** |
...
|---|---:|---:|---:|---:|---:|---:|
| **PyTorch fast path** (this project, AMD ROCm gfx1100) | **92.451** | 0.087 | 95.831 | 90.221 | 93.38 | 0.1435 |
...
*\*Baidu self-report from [arxiv:2606.23050](https://arxiv.org/abs/2606.23050) — **not on the OmniDocBench leaderboard and not independently reproduced** by anyone. Our **92.451** (PyTorch fast path, pinned weights `84757cb0`, torch 2.10+rocm7.0) is a controlled, reproducible measurement (committed manifest, gate PASS), up **+0.481** vs the prior 91.97 baseline with all modules ≥ baseline. The gain is env+weights + the decode_bpe postprocess fix, not batching luck — the Task-8 identity gate confirmed fast ≈ direct (Δ=0.0 exact post-fix) on the same env.*

**Honest parity framing.** The ~1.47pt gap to Baidu's 93.92 is **~entirely Text EditDist**. An evidence-based attribution ([`docs/parity/text-editdist-rootcause-2026-07-12.md`](docs/parity/text-editdist-rootcause-2026-07-12.md) — official-scorer dump of all 11,219 text blocks across 1,557 pages) localizes it as **~80% inherent**: ~65% genuine recognition divergence (the model's char-level ceiling vs this GT), ~14% math-style residual (the model emits semantically-correct \(...\), \sin where GT uses $...$, \operatorname{s i n}; CDM 0.958 confirms the math is right — char-level EditDist penalizes delimiter/spacing), and ~8% dense over-generation. Only ~6% is fixable (truncation / looping / `[Non-Text]` markers); the `[Non-Text]` strip is applied (PR #62, +0.0196 Overall) and a per-page looping retry regressed and was reverted. Block-level: 77.9% of blocks have edit_ratio <0.05; 66.1% are exact matches — divergence is spread thin across near-correct blocks, not concentrated garbage. The one open variable is checkpoint revision (we pin `84757cb0`; whether that matches Baidu's 93.92 run is unconfirmed — see [`docs/upstream/issue-66-text-gap-engagement-2026-07-15.md`](docs/upstream/issue-66-text-gap-engagement-2026-07-15.md)). Full diagnosis: [docs/PARITY.md](docs/PARITY.md).
