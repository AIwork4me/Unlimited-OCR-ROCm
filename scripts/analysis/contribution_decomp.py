...
for b in bins["bins"]:
    ...
    print(f"  [{b['bin'][0]:.2f},{b['bin'][1]:.2f})  n={b['count']:4d}  contrib={b['contribution_to_mean']:.4f}  ({pct_of_gap:5.1f}% of gap)")
print(f"\nIf failure tail (>0.5, n={fail_count}) fixed to paper 0.042 -> mean ~ {ceiling_if_failures_fixed:.4f}  (still {ceiling_if_failures_fixed-paper:+.4f} vs paper)")
print(f"  => D1 (looping truncation) can recover ~{fail_contrib:.4f} of the {gap:.4f} gap; the moderate tail [0.1,0.5) is the residual mystery (backend/genuine).")
