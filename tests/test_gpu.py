...
from rocm_ocr.gpu import assert_rocm, hip_visible_devices, set_hip_devices


def test_detect_rocm_positive(monkeypatch):
    monkeypatch.setattr("rocm_ocr.gpu.detect_rocm", lambda: True)
    from rocm_ocr.gpu import detect_rocm
...
def test_assert_rocm_passes(monkeypatch):
    monkeypatch.setattr("rocm_ocr.gpu.detect_rocm", lambda: True)
    assert_rocm()
...
def test_set_hip_devices(monkeypatch):
